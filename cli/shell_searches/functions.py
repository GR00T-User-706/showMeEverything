...
def search_shell_functions(pattern=''):
    if config.CURRENT_SHELL=='zsh': cmd=[config.CURRENT_SHELL,'-ic','print -rl -- '+'$'+'{(k)functions}']
    elif config.CURRENT_SHELL=='bash': cmd=[config.CURRENT_SHELL,'-ic','compgen -A function']
    else: return
